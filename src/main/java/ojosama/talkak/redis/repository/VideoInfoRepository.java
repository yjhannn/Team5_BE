...
public class VideoInfoRepository {
    ...
    public VideoInfoDto save(Long categoryId, Long videoId, VideoInfoDto videoInfoDto) {
        String key = VideoKey.VIDEO_INFO.generateKey(categoryId, videoId);
        redisService.setHashOps(key, hashConverter.toMap(videoInfoDto));
        return hashConverter.FromMap(redisService.getHashOps(key), VideoInfoDto.class);
    }

    public VideoInfoDto findByCategoryAndVideoId(Long categoryId, Long videoId) {
        String key = VideoKey.VIDEO_INFO.generateKey(categoryId, videoId);
        return hashConverter.FromMap(redisService.getHashOps(key), VideoInfoDto.class);
    }

    public VideoInfoDto updateViewCount(Long categoryId, Long videoId, Long updatedViewCount) {
        String key = VideoKey.VIDEO_INFO.generateKey(categoryId, videoId);
        redisService.setHashValue(key, VideoHashKey.VIEW_COUNT.getKey(), updatedViewCount);
        return hashConverter.FromMap(redisService.getHashOps(key), VideoInfoDto.class);
    }

    public VideoInfoDto updateLikeCount(Long categoryId, Long videoId, Long updatedLikeCount) {
        String key = VideoKey.VIDEO_INFO.generateKey(categoryId, videoId);
